...
public class StatusActionsFactory {
	...
	public Set<String> statusChange(StatusActions sa, String status, Set<String> metadataIds, String changeDate, String changeMessage) throws Exception {
        ...
        try {
            ...
            System.out.println("Status change failed. Cause: " + x.getCause().getMessage());
            x.getCause().printStackTrace();
            if(x.getCause().getCause() != null && x.getCause().getCause() instanceof StatusChangeException) {
                throw (StatusChangeException) x.getCause().getCause();
            }
            else {
            throw x;
        }

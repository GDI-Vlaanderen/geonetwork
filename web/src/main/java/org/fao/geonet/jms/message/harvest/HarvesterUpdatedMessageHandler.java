 ...
 */
public class HarvesterUpdatedMessageHandler implements MessageHandler {

    private ServiceContext context;
    
    public HarvesterUpdatedMessageHandler(ServiceContext context) {
        this.context = context;
    }
    
    public void process(String msg) throws ClusterException{
        Log.debug(Geonet.CLUSTER, "HarvesterUpdatedMessageHandler processing message '" + msg + "'");

        HarvesterUpdatedMessage harvesterMessage = new HarvesterUpdatedMessage();
        ...
        if(harvesterMessage.getSenderClientID().equals(ClusterConfig.getClientID())) {
            Log.debug(Geonet.CLUSTER, "HarvesterUpdatedMessageHandler ignoring message from self");
        }
        // message was sent by another GN instance
            ...
            try {
                ...
                Log.debug(Geonet.CLUSTER, "HarvesterUpdatedMessageHandler reinitializing harvestmanager");
                ...
                harvestManager.reinitialize(harvesterMessage.getId());
            }
            catch(Exception x) {
                Log.error(Geonet.CLUSTER, "Error processing HarvesterUpdatedMessageHandler message: " + x.getMessage());
                x.printStackTrace();

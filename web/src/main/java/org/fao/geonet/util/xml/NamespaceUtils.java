...
/**
 * Adaptation of JDOM2's NamespaceAware implementation in Element, to apply to JDOM1 Elements. To avoid a need to wrap
 * JDOM1 Element everywhere in GN, the methods are adapted to take an Element as parameter instead.
 *
 * Adapted code from https://github.com/hunterhacker/jdom/blob/master/core/src/java/org/jdom2/Element.java.
 *
 * When GeoNetwork upgrades  to use JDOM2, this class becomes obsolete.
 *
...
public class NamespaceUtils {
    ...
    /**
     * Get the Namespaces that are in-scope on this Element. Element has the most complex rules for the
     * namespaces-in-scope.
     * <p>
     ...
     * <li>The element's namespace is added (commonly {@link Namespace#NO_NAMESPACE})
     * <li>All the attributes are inspected and their Namespaces are included
     * <li>All Namespaces declared on this Element using addNamespaceDeclaration(Namespace) are included.
     * <li>If the element has a parent then the parent's Namespace scope is inspected, and any prefixes in the parent
     * scope that are not yet bound in this Element's scope are included.
     * <li>If the default Namespace (the no-prefix namespace) has not been encountered for this Element then
     * {@link Namespace#NO_NAMESPACE} is included.
     * </ul>
     * The Element's Namespace scope consist of it's inherited Namespaces and any modifications to that scope derived
     * from the Element itself. If the element is detached then it's inherited scope consists of just
     ...
     * <p>
     * The contents of this list will always be a subset (but in the same order) of getNamespacesInScope(), and will
     * never intersect getNamspacesIntroduced().
     *
    ...
    /**
     * Obtain a list of all namespaces that are introduced to the XML tree by this node. Only Elements and Attributes
     * can introduce namespaces, so all other Content types will return an empty list.
     * <p>
     * The contents of this list will always be a subset (but in the same order) of getNamespacesInScope(), and will
     * never intersect getNamspacesInherited().
     *
     * @return a read-only list of Namespaces.
     */

...
public class Geonetwork implements ApplicationHandler {
	...
	public Object start(Element config, final ServiceContext context) throws Exception {
        ...
        // Validationhook
        String validationHookClassName = handlerConfig.getValue(Geonet.Config.VALIDATIONHOOK_CLASS);
        Class validationHookClass = Class.forName(validationHookClassName);
        ...
        initLogo(servletContext, settingMan, dbms, context.getAppPath());
...
//		String schemaCatalogueFile = handlerConfig.getMandatoryValue(Geonet.Config.CONFIG_DIR) + File.separator + Geonet.File.SCHEMA_PLUGINS_CATALOG;
...
//                if (versionNumber > from && versionNumber <= to) {
			...
			// Update the logo 
			// TODO heikki: i tink not necessary here: initLogo(servletContext, dbms, appPath);
	...
	/**
	 * Updates site id if necessary, and copies the default dummy logo to the logo folder based on site id.
     *
     * @param servletContext
     * @param dbms
     * @param appPath
	 */
	private void initLogo(ServletContext servletContext, SettingManager sm, Dbms dbms, String appPath) {
		try {

...
<xsl:stylesheet version="2.0"
	...
	<xsl:variable name="background-color-banner">
		#ffffff
	</xsl:variable>
	<xsl:variable name="background-color-thumbnail">
		#f1f2f3
	</xsl:variable>
	...
	<xsl:variable name="header-border">
		2pt solid #2e456b
	</xsl:variable>
	...
	<xsl:variable name="header-color">
		#2e456b
	</xsl:variable>
	...
	<!-- ====================================================== Page footer 
		with node info, date and paging info -->
...
<!--
	...
	<xsl:template name="complexElementFop">
		...
		<!-- used as do*ElementAction url anchor to go back to the same position 
						...
						<fo:table-body>
							<fo:table-row border-top-style="solid"
								...
								<fo:table-cell padding-left="4pt" padding-right="4pt"
									padding-top="4pt" margin-top="4pt" number-columns-spanned="3">
	...
	<xsl:template name="blockElementFop">
		...
		<fo:table-row>
			<fo:table-cell padding-left="4pt" padding-right="4pt"
				padding-top="4pt" margin-top="4pt" number-columns-spanned="2">
				<fo:block>
					<xsl:if test="$block != ''">
						<xsl:if test="$label!=''">
		...
		<xsl:for-each select="$res/*">
			...
			<xsl:if test="$metadata/geonet:info/id != ''">
				<fo:table-row border-top-style="solid"
					...
					border-left-color="{$background-color}">
					<fo:table-cell padding-left="4pt" padding-right="4pt"
						padding-top="4pt" margin-top="4pt">
						<fo:block>
							<fo:external-graphic content-width="35pt">
								<xsl:attribute name="src"> url('<xsl:value-of
									select="concat($server/protocol, '://', $server/host,':', $server/port, $gui/url, '/images/logos/', $source , '.gif')" />')" </xsl:attribute>
							</fo:external-graphic>
						</fo:block>
					</fo:table-cell>
				...
				<fo:table-row border-bottom-style="solid"
					border-right-style="solid" border-left-style="solid"
					border-bottom-color="{$background-color}" border-right-color="{$background-color}"
					border-left-color="{$background-color}">
					<fo:table-cell number-columns-spanned="2">
						<fo:block margin-left="2pt" margin-right="4pt"
													...
													<fo:table-body>
														<xsl:call-template name="info-rows">
															...
															<xsl:with-param name="value"
																select="$metadata/geonet:info/uuid" />
														</xsl:call-template>


														<xsl:call-template name="info-rows">
															<xsl:with-param name="label"
																select="$gui/strings/abstract" />
														...
														<xsl:call-template name="info-rows">
															...
															<xsl:with-param name="value"
																select="$metadata/geonet:info/schema" />
														</xsl:call-template>
		        ...
		        <xsl:choose>
					...
					<xsl:otherwise>
						<fo:external-graphic content-width="4.6cm">
							<xsl:attribute name="src">
	                		<xsl:text>url('</xsl:text>
	                		<xsl:value-of select="concat($server/protocol, '://', $server/host,':', $server/port, $metadata/image[count($metadata/image)])"/>
	                		<xsl:text>')"</xsl:text>
	              			</xsl:attribute>
						</fo:external-graphic>
					</xsl:otherwise>
	...
	<!-- ==================================== List of metadata resources based 
		on online source section. Metadata must be in brief format -->
	<xsl:template name="metadata-resources">
		...
		<xsl:call-template name="info-rows">
			<xsl:with-param name="label"
				select="if ($title) then $gui/strings/resources else ''" />
	...
	<xsl:template name="banner">
		<fo:table table-layout="fixed" width="100%">
			...
			<fo:table-body>
				<fo:table-row border-bottom-style="solid"
					border-bottom-color="{$header-color}" border-bottom-width="1pt">
					<fo:table-cell display-align="center"
...
-->
					...
					<!-- <fo:table-cell display-align="right" text-align="top" background-color="{$background-color-banner}"> 
						...
						color="{$header-color}" font-weight="{$header-weight}" padding-top="4pt" 
						padding-bottom="4pt" padding-right="4pt" padding-left="4pt"> <xsl:value-of 
						select="upper-case(/root/gui/strings/searchResult)"/> </fo:block> </fo:table-cell> -->
				</fo:table-row>
			</fo:table-body>
		</fo:table>
	</xsl:template>

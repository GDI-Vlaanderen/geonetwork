...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="node">
        ...
        <xsl:variable name="schematrons"  select="$con/schematrons/children"/>
		...
		<node id="{@id}" type="{value}">
			...
			<content>
                ...
                <xsl:apply-templates select="$schematrons"  mode="schematrons"/>
    ...
    <xsl:template match="*" mode="schematrons">
        <schematrons>
            <xsl:for-each select="schematron">
                <schematron schemaId="{children/schemaId/value}" schematron="{children/schematron/value}" />
            </xsl:for-each>
        </schematrons>
    </xsl:template>

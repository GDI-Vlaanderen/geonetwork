									...
									<xsl:choose>
										...
										<xsl:when test="$metadata/image[@type='thumbnail']">
											<xsl:choose>
												...
												<xsl:otherwise>
													<img src="{$siteURL}{$metadata/image[@type='thumbnail']}" alt="{/root/gui/strings/thumbnail}"/>
												</xsl:otherwise>
											</xsl:choose>
										</xsl:when>
										<!-- no thumbnail -->
...
<!--							</xsl:if> -->
	...
	<xsl:template name="lookat" match="*">
		<LookAt>
			...
			<range>
				<xsl:variable name="vert"><xsl:value-of select="translate(string(round(northBL - southBL) * 90000),'-','')"/></xsl:variable>
				<xsl:variable name="hori"><xsl:value-of select="translate(string(round(eastBL - westBL) * 90000),'-','')"/></xsl:variable>
				<xsl:choose>

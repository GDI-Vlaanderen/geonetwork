		...
		<email>
			...
			<message>
				...
				<xsl:if test="$status='2' or $status='9'">
						<xsl:text>&#10;&#13;Voor meer informatie kan je steeds terecht op onze website </xsl:text><xsl:value-of select="/root/siteUrl"/><xsl:text>, of mail naar helpdeskGIS@vmm.be.</xsl:text>
				</xsl:if>

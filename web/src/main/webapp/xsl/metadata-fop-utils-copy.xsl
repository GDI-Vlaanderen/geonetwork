...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="newBlock">
		...
		<xsl:if test="$content and $content!=''">
			<fo:block-container width="100%" margin-left="10pt">
	...
	<xsl:template mode="elementFop" match="*|@*">
		...
		<xsl:choose>
			...
			<xsl:when
				...
				<xsl:if test="$table!=''">
					<fo:table width="95%" table-layout="fixed" border-top=".1pt solid {$title-color}">
						<xsl:choose>
						...
						<fo:table-body>
							<fo:table-row background-color="{$background-color}">
								<fo:table-cell border-right=".1pt solid {$title-color}">
									<fo:block>
										<fo:inline font-size="{$title-size}" font-weight="{$title-weight}"

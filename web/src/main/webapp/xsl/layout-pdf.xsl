...
<xsl:stylesheet version="2.0"
	...
	exclude-result-prefixes="fo" xmlns:gmd="http://www.isotc211.org/2005/gmd"
	...
	<!--<xsl:variable name="server" select="concat('http://', /root/gui/env/server/host, 
		':', /root/gui/env/server/port)"/> -->
	<xsl:variable name="siteURL"
		select="substring-before(/root/gui/siteURL, '/srv')" />
	...
	<xsl:template mode="schema" match="*">
		<xsl:choose>
			...
			<xsl:when
				test="local-name(.)='MD_Metadata' or local-name(..)='MD_Metadata'">
				iso19139
			</xsl:when>
			<xsl:otherwise>
				UNKNOWN
...
<!-- 
			...
			<!-- <fo:table width="100%" table-layout="fixed" background-color="#0f0"> 
				<fo:table-column column-width="5cm" /> <fo:table-column column-width="15cm" 
				/> <fo:table-body> <xsl:variable name="schemaTemplate" select="concat('metadata-fop-',$schema)" 
				/> <saxon:call-template name="{$schemaTemplate}"> <xsl:with-param name="schema" 
				select="$schema" /> <xsl:fallback> <xsl:message> Fall back as no saxon:call-template 
				exists </xsl:message> </xsl:fallback> </saxon:call-template> </fo:table-body> 
				</fo:table> -->
		</xsl:for-each>
	</xsl:template>

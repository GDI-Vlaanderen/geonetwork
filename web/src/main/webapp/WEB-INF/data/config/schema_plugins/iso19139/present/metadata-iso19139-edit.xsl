    ...
    <xsl:template mode="iso19139" match="gmd:DQ_DataQuality|gmd:identificationInfo|gmd:distributionInfo|gmd:thesaurusName|
        ...
        <xsl:if test="name(.)='gmd:dataQualityInfo'">
	        <xsl:if test="gmd:DQ_DataQuality/gmd:report/gmd:DQ_DomainConsistency[contains(gmd:result/gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:title/gco:CharacterString,'2007/2/E')]">
		        <xsl:apply-templates mode="iso19139Report" select="gmd:DQ_DataQuality/gmd:report/gmd:DQ_DomainConsistency[contains(gmd:result/gmd:DQ_ConformanceResult/gmd:specification/gmd:CI_Citation/gmd:title/gco:CharacterString,'2007/2/E')]">
		            <xsl:with-param name="schema" select="$schema"/>
		            <xsl:with-param name="edit"   select="$edit"/>
		            <xsl:with-param name="title">
...
to build the XML fragment in the editor. -->
  ...
  <xsl:template mode="getThesaurusId" match="*">
  	<xsl:choose>
  		...
  		<xsl:when test="contains(normalize-space(gmd:thesaurusName/gmd:CI_Citation/gmd:title/gco:CharacterString),'INSPIRE') and contains(normalize-space(gmd:thesaurusName/gmd:CI_Citation/gmd:title/gco:CharacterString),'feature concept')">geonetwork.thesaurus.external.theme.featureconcept</xsl:when>
  		<xsl:when test="contains(normalize-space(gmd:thesaurusName/gmd:CI_Citation/gmd:title/gco:CharacterString),'priority data set')">geonetwork.thesaurus.external.theme.PriorityDataset</xsl:when>

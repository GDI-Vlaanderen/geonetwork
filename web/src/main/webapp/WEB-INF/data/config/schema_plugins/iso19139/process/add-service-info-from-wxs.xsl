...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="setAndReplaceOperationsMode" select="geonet:parseBoolean($setAndReplaceOperations)"/>
  ...
  <xsl:template name="analyze-add-service-info-from-wxs">
    ...
    <xsl:if test="$srv"><!-- Only apply to service metadata-->
      <xsl:for-each select="$onlineResources">
        <suggestion process="add-service-info-from-wxs" id="{generate-id()}-service" category="onlineSrc" target="srv:containsOperations">
          <name><xsl:value-of select="geonet:i18n($wxs-info-loc, 'a', $guiLang)"/><xsl:value-of select="./gmd:linkage/gmd:URL"
          /><xsl:value-of select="geonet:i18n($wxs-info-loc, 'b', $guiLang)"/>.</name>
          <operational>true</operational>
          <params>{ setAndReplaceOperations:{type:'boolean', defaultValue:'<xsl:value-of select="$setAndReplaceOperations"/>'},
      ...
      <xsl:variable name="ows">
        <xsl:choose>
          <xsl:when test="(local-name($wxsCapabilitiesDoc/.)='WFS_Capabilities' and namespace-uri($wxsCapabilitiesDoc/.)='http://www.opengis.net/wfs' 
            ...
            or (local-name($wxsCapabilitiesDoc/.)='Capabilities' and namespace-uri($wxsCapabilitiesDoc/.)='http://www.opengis.net/wps/1.0.0')">true</xsl:when>

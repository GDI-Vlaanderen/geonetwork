...
<xsl:stylesheet version="2.0" xmlns:xsl ="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template mode="iso19139" name="file-upload" match="*[gmx:FileName]">
    ...
    <xsl:call-template name="file-or-logo-upload">
      ...
      <xsl:with-param name="action" select="concat('startFileUpload(', $apos, /root/*[name(.)='gmd:MD_Metadata' or @gco:isoType='gmd:MD_Metadata']/geonet:info/id, $apos, ', ', $apos, gmx:FileName/geonet:element/@ref, $apos, ');')"/>
    </xsl:call-template>
  </xsl:template>

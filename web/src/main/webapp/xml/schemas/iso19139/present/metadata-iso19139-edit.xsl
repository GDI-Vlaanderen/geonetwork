...
<!-- metadata Extension Information - dead last because its boring and
  ...
  <xsl:template mode="iso19139" match="gmd:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'OGC:WMS-') and contains(gmd:protocol/gco:CharacterString,'-get-map') and gmd:name]" priority="2">
    ...
    <xsl:choose>
      ...
      <xsl:when test="string(ancestor-or-self::*[name(.)='gmd:MD_Metadata' or @gco:isoType='gmd:MD_Metadata']/geonet:info/dynamic)='true' and string($name)!='' and string($linkage)!=''">
  ...
  <xsl:template mode="iso19139" match="gmd:CI_OnlineResource[starts-with(gmd:protocol/gco:CharacterString,'WWW:DOWNLOAD-') and contains(gmd:protocol/gco:CharacterString,'http--download') and gmd:name]" priority="2">
    ...
    <xsl:choose>
      ...
      <xsl:when test="string(ancestor-or-self::*[name(.)='gmd:MD_Metadata' or @gco:isoType='gmd:MD_Metadata']/geonet:info/download)='true' and string($linkage)!='' and not(contains($linkage,$download_check))">
        <xsl:apply-templates mode="simpleElement" select=".">
          <xsl:with-param name="schema"  select="$schema"/>
        ...
        <xsl:apply-templates mode="simpleElement"
          ...
          <xsl:with-param name="text">&#160;
            ...
            <xsl:variable name="url" select="if (contains($fileName, '://')) 
                                                then $fileName 
                                                else geonet:get-thumbnail-url($fileName, ancestor-or-self::*[name(.)='gmd:MD_Metadata' or @gco:isoType='gmd:MD_Metadata']/geonet:info, /root/gui/locService)"/>

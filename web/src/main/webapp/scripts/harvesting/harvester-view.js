...
this.setDataCommon = function(node)
{
    ...
    this.unselectSchematrons();
    this.selectSchematrons(content);
...
this.getDataCommon = function()
{
		...
		data =
	{
        ...
        SCHEMATRONS  : this.getSelectedSchematrons()
	}
	}
	else {
		data =
...
this.getSelectedSchematrons = function()
{
    var data = [];
    var list = $(prefix+'.schematrons').getElementsByTagName('TR');
    ...
    for (var i=1; i<list.length; i++)
    {
        ...
        for (var j=0; j<inputList.length; j++) {
            //-- the string is '<prefix>.schema.schematron.xsl'
            var schemaId = inputList[j].getAttribute('id').split('.')[1];
            var schematron = inputList[j].getAttribute('id').split('.')[2] + "." +  inputList[j].getAttribute('id').split('.')[3];
...
this.unselectSchematrons = function()
{
    var ctrl = $$('input.schematron');
...
this.selectSchematrons = function(node)
{
    var schematrons = node.getElementsByTagName('schematrons');

    if (schematrons.length == 0)
        return;

    var list = schematrons[0].getElementsByTagName('schematron');

    for (var i=0; i<list.length; i++)
        selectSchematron(list[i]);
}
...
function selectSchematron(schematron)
{
    var id   = prefix+'.'+schematron.getAttribute('schemaId')+'.'+schematron.getAttribute('schematron');
    if ($(id)) {
        $(id).checked = true;
    }
}

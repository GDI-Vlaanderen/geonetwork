...
{
    var metadataId = $('editForm').id.value;
    ...
    var myExtAJaxRequest = Ext.Ajax.request({
        ...
        success: function(result, request) {
            ...
            if (html.blank()) { // more than one left, no child-placeholder returned
                ...
                if (bottomElement(thisElement) && $('editForm').currTab.value!='simple') {
...
function doSaveAction(action,validateAction)
{
    ...
    if (typeof validateAction != 'undefined') {
        $('editForm').showvalidationerrors.value = "true";
    } else {
        $('editForm').showvalidationerrors.value = "false";
    }
        ...
        var myAjax = new Ajax.Updater(
            ...
            getGNServiceURL(action),
            {
                ...
                onComplete: function(req) {
                    if (req.status == 200) {
                        if ($('editForm').showvalidationerrors.value=='true')
                            getValidationReport();
                        setBunload(true); // reset warning for window destroy
                        initCalendar();
                        validateMetadataFields();
...
function doFileRemoveAction(action, ref, access, id)
{
    ...
    var top = findPos($(id));
    setBunload(false);
    $('editForm').access.value = access;
    $('editForm').ref.value = ref;
    $('editForm').position.value = top;
    $('editForm').action = action;
    goSubmit('mainForm');
}
                ...
                if (single) {
                    if (this.ref != null) {
                        ...
                        if (this.mode=='iso19110') {
                            ...
                            var url = 'xml.relation.insert?parentId=' + $('editForm').id.value +
 ...
 * @param id
 */
function detachFeatureCatalogueMd(parentUuid, childUuid, parentId) {
...
function computeExtentFromKeywords(mode) {
    window.location.replace("metadata.processing?id=" + $('editForm').id.value

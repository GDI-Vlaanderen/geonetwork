	...
	<xsl:template name="getButtons">
		...
		<span id="buttons_{$id}">
			...
			<xsl:if test="name(.)='gmd:descriptiveKeywords'">
				<xsl:text> </xsl:text>
				<a id="keywordHelpLink_{$id}" style="cursor:hand;cursor:pointer;" href="{/root/gui/url}/vmm_werkwijze_invullen_metadata.pdf" target="_blank"><img src="{/root/gui/url}/images/info_small.png"/></a>
			</xsl:if>
			<!-- xsd and schematron validation error button -->
			<xsl:if test="normalize-space($validationLink)">
				<xsl:text> </xsl:text>
		        <div style="display:none;" class="{normalize-space($validationLink)}"/>
<!--
